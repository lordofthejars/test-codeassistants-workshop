...
To configure IBM Bob to use the MCP Server, go to the `.bob` directory (it is already created and has already some content there), and create a `mcp.json` file with the following content:
...
[quote, IBM BOB]
----
You are an expert tester, and you need to do some exploratory testing plan for my web application hosted at http://localhost:5173/

The application is a webpage for booking intergalactic trips, so you can expect flights, bookings, different kinds of trips (economy, business, ...), and booking extras such as heavy luggage.
...
After some minutes, and after watching the browser do things automatically, we end up with an exploratory testing plan file that explains all the steps you can execute and some open questions that could be addressed.

== Implementing Test Plan

For now, let's leave the test plan as is. We can get feedback from the Code Assistant and ask it to improve it further, but for the sake of simplicity, let's ask the Code Assistant to implement the exploratory test.

WARNING: This step consumes a lot of tokens. You can skip it if you only have a few tokens left to run the last example in the workshop.

[quote, IBM BOB]
----
You are an expert Java developer who knows how to write Playwright tests.

Implement only the exploratory test plan created in the previous step using the Playwright Java framework and following the best practices, such as the PageObjects pattern.
The project already has the Playwright dependency registered, so the version is fixed and doesn't need to change or be configured.
