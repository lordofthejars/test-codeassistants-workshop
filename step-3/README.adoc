...
The code assistant creates a new file `USER_RESOURCE_NEGATIVE_TEST_CASES.md` containing the negative tests.
...
As in the previous case, the code assistant creates a new class file and runs verification to ensure it works.
...
Code Assistant creates a new file that may contain regressions.
...
[quote, IBM BOB]
----
A User reported that when searching for a booking with a negative userId, the application throws an exception. Can you create a reproducer?
...
Developers always strive to write as many tests as possible, but it is difficult to cover all use cases.
We can use code assistants to improve the test coverage.

[quote, IBM BOB]
----
As a Tester, based on the UserResourceNegativeTests, can you identify gaps and areas of inadequate coverage? Suggest additional tests to ensure comprehensive coverage.
----
...
In some cases, we want to improve test coverage by inspecting the code, reviewing previous test coverage reports, and identifying ways to improve.
...
Read the JaCoCo file, explore what code has already been tested, and what tests could be done to improve the coverage. 
...
Performance testing is also an important part of most applications; there are many frameworks for implementing it, such as JMeter, Gatling (in Java), and Locust. 
But the important thing here is to understand what the acceptable values are for each case and which thresholds are acceptable and which are not.
...
Ideally, we should have BDD specs in place before any implementation; however, we are in a rush, or it is legacy code, so no specs have been written yet.
Not all projects have a way to verify that the application is doing what the business mandates, even though, of course, we know it is because it has been running for years. 

But this is not an excuse to not have a minimal set of tests to verify its behavior.

[quote, IBM BOB]
----
You are a tester engineer who understands code. You need to understand what FlightResource does, then generate a Gherkin specification file with features, scenarios, and Given/When/Then steps that explain your findings and propose test data.
----
...
The specification includes comprehensive test scenarios; in practice, you should validate that all assumptions are correct and remove those that don't make sense.

After this, push the `Start New Task` button to start a new conversation.

== Security

Security tests are also a type of test that requires collaboration among testers, the security team, and developers.
Now, you can use the code assistant with Skills to verify the application's security and write tests to ensure it does not happen.
...
for some form of injection, and it has the most CVEs of any category.
...
[quote, IBM BOB]
----
You are a test engineer. Can you write tests for the high-severity issues? The idea is to have tests that represent what an attacker might use to exploit the application.
----
...
== Optimazing Execution

As testers, we design tests, perform exploratory testing, define the testing strategy, and provide an optional test execution plan for the given CPUs or Machines.

Close the current `IBM Bob` project and open this `workshop` directory.
You'll see that there is a `surefire.xml` file that represents a test execution report when using Maven.

Let's ask IBM Bob to find the best schedule:

[quote, IBM BOB]
----
You are a process scheduler planner. Your task is to read a surefire Junit execution and, for each class's time execution, find the best execution order of test classes to maximize time execution if the computer has only 4 CPUs.
----

IBM Bob will present a report on the optimal order for executing tests and a Gantt diagram to visualize the execution.

If you want to visualize, go to https://mermaid.live/ and copy the content.

image::gantt.png[]

These tests are white-box tests, or tests that verify a part of the application. We'll start doing more black-box testing to verify the entire web application.
